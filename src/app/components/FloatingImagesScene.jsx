// FloatingImagesScene.jsx - Heavily Optimized
'use client';
...
function Starfield({ count = 200 }) {
  ...
  const { positions, colors, starTexture } = useMemo(() => {
    ...
    // Simplified star positioning - less computation
    for (let i = 0; i < count; i++) {
      ...
      // Only two color types for better performance
      if (i % 2 === 0) {
...
function FloatingImage({
  ...
  // Simplified geometry - lower resolution
...
function SphericalGallery({
  ...
  useFrame(() => {
    ...
    // Update camera and lookAt less frequently
...
export default function FloatingImagesScene() {
  ...
  const handleImageClick = ({ href }) => (window.location.href = href);

  // Batch prefetch on idle
  useEffect(() => {
    const prefetch = () => {
